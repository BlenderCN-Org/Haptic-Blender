...
PyObject *initGameLogic(KX_KetsjiEngine *engine, KX_Scene* scene) // quick hack to get gravity hook
{
	...
	for (int i=0; i<JOYINDEX_MAX; ++i) {
		...
		if (omni && omni->Connected()) {
			...
			PyObject* tmp = gp_PythonOmni[i]->NewProxy(true);
			Py_INCREF(tmp);
			PyList_SET_ITEM(omnilist, i, tmp);
		} else 	{
			omni->ReleaseInstance();
			Py_INCREF(Py_None);
			PyList_SET_ITEM(omnilist, i, Py_None);
		}
	}
	PyDict_SetItemString(d, "omnis", omnilist);
